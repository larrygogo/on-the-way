...
import { Collision, Rect, Point } from '../../engine/physics/Collision';
...
export class Enemy implements Renderable {
  ...
  /**
   * 获取脚底碰撞盒矩形（公开方法，返回对象字面量）
   */
  getFootprintRectPublic(): { x: number; y: number; width: number; height: number } {
    const rect = this.getFootprintRect();
    return {
      x: rect.x,
      y: rect.y,
      width: rect.width,
